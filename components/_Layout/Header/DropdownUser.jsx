...
import { camelCaseToCapitalizedString } from '@/services/string-functions';

const DropdownUser = () => {
  const [dropdownOpen, setDropdownOpen] = useState(false);
  const [profileImage, setProfileImage] = useState(
    `${process.env.NEXT_PUBLIC_BE_HOST}/static/users/_default.png`
  );
  ...
  // get profile image
  useEffect(() => {
    setProfileImage('images/icon/group.svg');
    // if (session) {
    //   if (session?.user?.image) {
    //     fetch(
    //       `${session?.user?.image}`,
    //       // `${process.env.NEXT_PUBLIC_BE_HOST}/static/users/${session?.user?.image}`,
    //       { method: 'HEAD' }
    //     ).then((res) => {
    //       if (res.ok) {
    //         setProfileImage(
  ...
  return (
    <div className="relative ml-5">
      ...
      <div
        ...
        <div className="my-2 mx-5 text-xs">
          <span className="font-bold">Roles: </span>
          {session &&
            session?.user?.roles
              ?.filter((role) => role !== 'user')
              .map((role) => camelCaseToCapitalizedString(role))
              .join(',')}
        </div>
        <ul className="flex flex-col gap-5 border-b border-stroke px-6 py-7.5 dark:border-strokedark">
          <li>
            <Link
              href={`/admin/users/profile/${session?.user?.uuid}`}
